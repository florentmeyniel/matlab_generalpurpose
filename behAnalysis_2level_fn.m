function [bEi bRi bE bR] = behAnalysis_2level_fn(EffortDur, RestDur, method, doplot, stat)
...
for i_type=1:2
    ...
    for i_sub=unique(data(:,1))'
        ...
        [b]=regress(data_sub(:,7), ...
            [X_cst X_RW X_FC X_bloc X_trial X_intrial]);
...
for i_type=1:2
    ...
    for i_sub=unique(data(:,1))'
        ...
        X_RWxtrial=zscore(X_RW.*X_trial);
        X_RWxbloc=zscore(X_RW.*X_bloc);
        X_RWxintrial=zscore(X_RW.*X_intrial);
        X_FCxtrial=zscore(X_FC.*X_trial);
        X_FCxbloc=zscore(X_FC.*X_bloc);
        X_FCxintrial=zscore(X_FC.*X_intrial);
        X_RWxFC=zscore(X_RW.*X_FC);
