...
% columns:
%   - subject number
%   - hand (1: opt.def_hand(1), 2: opt.def_hand(2))
%   - block number
%   - trial number
%   - reward level (1: lowest, 3:highest)
%   - force level (1: lowest, 3:highest)
%  (- Diplayed force level, if different from applied) 
%   - data, can be:
...
for sub = 1:length(subIDlist)
    for hand = 1:length(def_hands)
        ...
        if strcmp(def_blocks, 'default')
           blocks_list = 1: size(dir(strcat(datadir, FilePrefix,num2str(subIDlist(sub)), def_hands(hand), '*.mat')),1);
        else
        ...
        for block = blocks_list
            ...
            data = load(strcat(datadir, FilePrefix, ...
                num2str(subIDlist(sub)), def_hands(hand), 'bloc', num2str(block), '.mat'));
